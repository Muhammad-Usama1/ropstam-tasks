                                    ...
                                    <v-container>
                                        <v-row>
                                            <v-col cols="12" md="12" sm="6">
                                                ...
                                                <v-select v-model="selectedCategory" :items="categoryStore.categories" variant="outlined"   
                                                    label="Select Category" item-text="name" item-value="id"
                                                    outlined></v-select>
                                            </v-col>
                                            <v-col cols="12" md="12" sm="6">
...
import { useCategories } from '@/stores/categories/addnew'; // Import the category store
const vehicleStore = useVehicleList();
const categoryStore = useCategories(); // Initialize category store
...
const saveVehicle = async () => {
  ...
  if (isNewVehicle.value) {
    // Assign the selected category to the vehicle object
...
const deleteVehicle = (item) => {
    ...
    console.log("vehicle id is :", deleteVehicleId.value)
    process.value = false
...
const deleteVehicleConfirm = async () => {
    ...
    if (deleteVehicleId.value) {
        vehicleStore.deleteVehicle(deleteVehicleId.value); // Call delete action with the item ID
        text.value = vehicleStore.text
        await fetchVehicles()
        deleteVehicleId.value = null; // Reset the item ID after deletion
        process.value = false
        snackbar.value=true
    }
    dialogDelete.value = false; // Close delete confirmation dialog
};
// Updating the vehicle
const updateVehicleId = ref()
const editItem = (item) => {
    isNewVehicle.value = false;
    updateVehicleId.value = item._id;
    const foundVehicle = vehicles.value.find(v => v._id === item._id);
    console.log("vehicle data :", foundVehicle)
    if (foundVehicle) {
